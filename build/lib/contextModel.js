...
exports.classifyRegimes = classifyRegimes;
exports.contextSimilarity = contextSimilarity;
exports.weightObservations = weightObservations;
exports.weightedPrediction = weightedPrediction;
exports.detectConceptDrift = detectConceptDrift;
const clamp01 = (value) => Math.max(0, Math.min(1, value));
const cold = (temperature) => clamp01((10 - temperature) / 12);
const mild = (temperature) => clamp01(1 - Math.abs(temperature - 12) / 10);
const warm = (temperature) => clamp01((temperature - 16) / 10);
const sunny = (radiation) => clamp01((radiation - 80) / 420);
const highPv = (pvKW) => clamp01((pvKW - 1) / 7);
function classifyRegimes(context) {
...
function numericSimilarity(a, b, scale) {
    if (a === null || b === null)
        return 0.65;
    return Math.exp(-Math.abs(a - b) / scale);
}
function circularHourSimilarity(a, b) {
    const difference = Math.min(Math.abs(a - b), 24 - Math.abs(a - b));
    return Math.exp(-difference / 4);
}
function contextSimilarity(current, historical) {
    const components = [
...
function weightObservations(current, observations, recencyHalfLifeDays, outlierZScore = 3.5) {
...
function detectConceptDrift(residuals, minimumSamples, relativeBiasThreshold) {
